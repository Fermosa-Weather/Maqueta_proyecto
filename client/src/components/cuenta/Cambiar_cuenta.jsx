...
const handleAccountClick = (cuenta) => {
      ...
      <div className="text-sm text-gray-300">{userInfo.email}</div>
          ...
          {cuentas.map(cuenta => (
            <li key={cuenta._id} className="flex items-center space-x-3 p-2 rounded-md transition-all duration-200 cuenta cursor-pointer" onClick={() => handleAccountClick(cuenta)}>
              <img src={cuenta.fotoUser || "../../src/images/usuario.jpg"} alt="" className="w-16 h-16 rounded-full object-cover border-2 border-white shadow-sm" />
              <div className="flex flex-col">
  ...
  <ul className="space-y-2">
    {userInfo && userInfo._id ? (
      // Si tiene un id, mostramos el enlace para añadir otra cuenta
      ...
      // Si no tiene id (es la primera cuenta), lo redirigimos al login
      ...
      <div className="mt-2 border-t border-gray-200 pt-2">
        <ul className="space-y-2">
          <a href="#cerrar_sesion" onClick={handleLogout} className='no-underline'>
